...
## Production
Lets look at the Diagram 1 of the production of coffee between 1960 and 2015 crops years.
Here you can see the productivity (1000 tonns) and harvested area (1000 ha)[^1].
...
Let's try to check the lack of perfect multicollinearity in the predictors;

```{r multicollinearity, echo=FALSE, warning=FALSE}
pairs(rain_temp_crop[c(-1,-5)])
```

Obviously there ist no multicollinearity in the predictors.
...
and with interaction between the Precipitation (pr) and the temperature (t_cel) predictors (Table 2,  Model 2):
...
Let's build predictions based on these two models and show them on the Diagram 4: 
...
We can noted that substantial productivity was significantly increased between 2002 and 2012 (Diagram 1) and our predictions works after that not so good as before. But I assume, that after the boom of the technical progress the models will show better result or we need to include another predictor, like Investing in the agriculture sector. 
